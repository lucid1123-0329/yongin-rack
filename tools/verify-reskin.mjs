...
for (const file of htmlFiles) {
  ...
  const designAt = html.search(/href=["']\/?css\/design\.css\?v=77["']/);
  ...
  if (cacheVersions.length === 0 || cacheVersions.some((version) => version !== '77')) {
    themeProblems.push(file + ' cache version');
  }
}
report('HTML v3 theme wiring', themeProblems, htmlFiles.length + ' files use Pretendard, v3 colors, and v77');
...
if (String(versionData.version) === '77') pass('version.json cache version', 'version is 77');
else fail('version.json cache version', 'expected 77, found ' + versionData.version);
...
const expectedSw = swBefore
  .replace("const CACHE_NAME = 'yr-v64';", "const CACHE_NAME = 'yr-v77';")
