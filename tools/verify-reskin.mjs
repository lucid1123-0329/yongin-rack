...
for (const file of htmlFiles) {
  ...
  const designAt = html.search(/href=["']\/?css\/design\.css\?v=82["']/);
  ...
  if (cacheVersions.length === 0 || cacheVersions.some((version) => version !== '82')) {
    themeProblems.push(file + ' cache version');
  }
}
report('HTML v3 theme wiring', themeProblems, htmlFiles.length + ' files use Pretendard, v3 colors, and v82');
...
if (String(versionData.version) === '82') pass('version.json cache version', 'version is 82');
else fail('version.json cache version', 'expected 82, found ' + versionData.version);
...
const expectedSw = swBefore
  .replace("const CACHE_NAME = 'yr-v64';", "const CACHE_NAME = 'yr-v82';")
