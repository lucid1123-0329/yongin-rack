...
for (const file of htmlFiles) {
  ...
  const designAt = html.search(/href=["']\/?css\/design\.css\?v=81["']/);
  ...
  if (cacheVersions.length === 0 || cacheVersions.some((version) => version !== '81')) {
    themeProblems.push(file + ' cache version');
  }
}
report('HTML v3 theme wiring', themeProblems, htmlFiles.length + ' files use Pretendard, v3 colors, and v81');
...
if (String(versionData.version) === '81') pass('version.json cache version', 'version is 81');
else fail('version.json cache version', 'expected 81, found ' + versionData.version);
...
const expectedSw = swBefore
  .replace("const CACHE_NAME = 'yr-v64';", "const CACHE_NAME = 'yr-v81';")
